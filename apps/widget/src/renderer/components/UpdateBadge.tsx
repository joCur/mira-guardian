  ...
  return (
    <div ref={box} className="relative" style={{ WebkitAppRegion: "no-drag" } as any}>
      <button onClick={() => setOpen(o => !o)} aria-expanded={open}
        ...
        className={`flex items-center gap-1.5 h-[26px] px-2.5 rounded-[7px] border text-xs font-semibold transition-colors ${tone}`}>
      ...
      {open && (
        <div className="absolute right-0 top-[32px] z-30 w-[296px] bg-ctp-mantle border border-ctp-surface1 rounded-[10px] shadow-lg shadow-ctp-crust/50 px-4 py-3.5">
          ...
          {ready && (
            <div className="text-xs text-ctp-subtext1 mt-2 leading-normal">
          ...
          <div className="flex items-center justify-between gap-2 mt-3">
            {status.notesUrl
              ? <button onClick={() => onOpenNotes(status.notesUrl!)}
                  className="text-xs text-ctp-blue hover:underline">Was ist neu ↗</button>
              : <span />}
            {ready && (
              <button onClick={onInstall}
                className="rounded-lg px-3.5 py-[7px] text-xs font-semibold bg-ctp-green/25 text-ctp-green border border-ctp-green/40 hover:bg-ctp-green/30 transition-colors whitespace-nowrap">
