...
export function FrontmatterCard({ fields }: { fields: FmField[] }) {
  ...
  return (
    <div className="bg-ctp-mantle border border-ctp-surface0 rounded-lg px-4 py-3 mb-4">
      ...
      {description?.newValues && <div className="text-[13px] text-ctp-text leading-snug mt-2">{description.newValues.join(", ")}</div>}
      {rest.length > 0 && (
        <div className={`grid grid-cols-[minmax(110px,auto)_1fr] gap-x-4 gap-y-1.5 ${hasHeader ? "mt-2.5 pt-2.5 border-t border-ctp-surface0" : ""}`}>
          {rest.map(field => (
            <React.Fragment key={field.key}>
              ...
              <span className="text-[10px] tracking-[0.08em] font-semibold uppercase text-ctp-subtext0 leading-[1.9] truncate">{field.key}</span>
              {field.newValues && field.newValues.length > 1 && !field.changed
                ? <span className="min-w-0 text-[12px] leading-relaxed">{field.newValues.map((v, i) => <span key={i} className="font-mono text-ctp-subtext1 block truncate">{v}</span>)}</span>
                : <span className="text-[12px] text-ctp-subtext1 leading-relaxed min-w-0"><Value field={field} /></span>}
            </React.Fragment>
          ))}
        </div>
      )}
