  ...
  return (
    <div className="flex-1 overflow-y-auto px-8 py-6">
      <div className="max-w-[640px] mx-auto">
        <div className="text-2xl font-bold text-ctp-text">Hüter</div>
        <div className="text-base text-ctp-subtext0 mt-1">Jede Änderung an der Memory-Bank braucht die Bestätigung aller verknüpften Hüter.</div>
        <div className="mt-[18px] flex flex-col gap-2">
            ...
            <div key={g.id} className="bg-ctp-mantle border border-ctp-surface0 rounded-[10px] px-4 py-3">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-base font-semibold text-ctp-text truncate">{g.name}</div>
                ...
                <button onClick={() => void relink(g)}
                  className="rounded-lg px-3 py-[6px] text-sm font-semibold whitespace-nowrap transition-colors bg-ctp-surface0/60 text-ctp-subtext1 border border-ctp-surface1 hover:text-ctp-text hover:border-ctp-overlay0 shrink-0">
                  Gerät verknüpfen
                </button>
              </div>
              {relinkError[g.id] && (
                <div className="text-sm text-ctp-red mt-2">{relinkError[g.id]}</div>
              )}
              {relinkCodes[g.id] && (
                <div className="mt-2.5 rounded-lg border border-ctp-blue/30 bg-ctp-blue/10 px-3.5 py-3">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="font-mono text-lg tracking-[0.15em] text-ctp-text bg-ctp-surface0 border border-ctp-surface1 rounded-md px-2.5 py-1">
                      {relinkCodes[g.id].code}
                    </span>
                    <span className="text-xs text-ctp-subtext0">gültig bis {zeitpunkt(relinkCodes[g.id].expiresAt)}</span>
                  </div>
                  <div className="text-sm text-ctp-subtext1 mt-2 leading-relaxed">
          ...
          {pending.map(p => (
            <div key={p.code} className="flex items-center gap-3 bg-ctp-base border border-dashed border-ctp-surface2 rounded-[10px] px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-base font-semibold text-ctp-subtext1 truncate">{p.name}</div>
              ...
              <span className="font-mono text-sm text-ctp-text bg-ctp-surface0 border border-ctp-surface1 rounded-md px-2 py-[3px] tracking-widest shrink-0">{p.code}</span>
            </div>
          ))}
        </div>
        <div className="mt-[22px] bg-ctp-mantle border border-ctp-surface0 rounded-[10px] px-[18px] py-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <input value={name} onChange={e => setName(e.target.value)} placeholder="Name"
              className="flex-1 min-w-[150px] bg-ctp-crust border border-ctp-surface1 focus:border-ctp-overlay0 rounded-lg text-base text-ctp-text placeholder:text-ctp-overlay0 px-3 py-2 outline-none" />
            <input value={email} onChange={e => setEmail(e.target.value)} placeholder="E-Mail"
              className="flex-1 min-w-[170px] bg-ctp-crust border border-ctp-surface1 focus:border-ctp-overlay0 rounded-lg text-base text-ctp-text placeholder:text-ctp-overlay0 px-3 py-2 outline-none" />
            <button disabled={!valid} onClick={() => { onInvite(name.trim(), email.trim()); setName(""); setEmail(""); }}
              className="rounded-lg px-[18px] py-2 text-base font-semibold whitespace-nowrap transition-colors bg-ctp-green/25 text-ctp-green border border-ctp-green/40 hover:bg-ctp-green/30 disabled:bg-ctp-surface0/40 disabled:text-ctp-overlay0 disabled:border-ctp-surface0 disabled:cursor-not-allowed">Zugangscode erzeugen</button>
          </div>
          ...
          <div className="flex flex-col gap-2">
            {devices.map(d => (
              <div key={d.id} className="flex items-center gap-3 bg-ctp-base border border-ctp-surface0 rounded-lg px-3.5 py-2.5">
                <div className="flex-1 min-w-0">
                  <div className="text-base font-semibold text-ctp-text truncate">{d.label}</div>
                ...
                {d.current
                  ...
                  : <button onClick={() => { setRevoking(d.id); void onRevoke(d.id).finally(() => setRevoking(null)); }}
                      ...
                      className="rounded-lg px-3 py-[6px] text-sm font-semibold whitespace-nowrap transition-colors bg-ctp-red/15 text-ctp-red border border-ctp-red/40 hover:bg-ctp-red/25 disabled:opacity-50 shrink-0">
                      Zugang entziehen
                    </button>}
              </div>
            ))}
        ...
        <div className="mt-[22px] bg-ctp-mantle border border-ctp-surface0 rounded-[10px] px-[18px] py-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="font-mono text-base text-ctp-subtext1 break-all flex-1 min-w-[200px]">{serverUrl}</span>
            <button onClick={() => setConfirmSignOut(true)}
              className="rounded-lg px-[18px] py-2 text-base font-semibold whitespace-nowrap transition-colors bg-ctp-red/15 text-ctp-red border border-ctp-red/40 hover:bg-ctp-red/25">Abmelden</button>
          </div>
          <div className="text-xs text-ctp-overlay0 mt-2 leading-normal">
          ...
          {confirmSignOut && (
            <div className="mt-3 rounded-lg border border-ctp-red/40 bg-ctp-red/10 px-3.5 py-3">
              <div className="text-base font-semibold text-ctp-red">Wirklich abmelden?</div>
              <div className="text-sm text-ctp-subtext1 mt-1 leading-relaxed">
              ...
              <div className="flex gap-2.5 justify-end mt-2.5">
                <button onClick={() => setConfirmSignOut(false)}
                  className="rounded-lg px-3.5 py-[7px] text-base text-ctp-subtext0 border border-ctp-surface1 hover:text-ctp-text transition-colors">Abbrechen</button>
                <button onClick={() => void onSignOut()}
                  className="rounded-lg px-4 py-[7px] text-base font-semibold bg-ctp-red/25 text-ctp-red border border-ctp-red/40 hover:bg-ctp-red/30 transition-colors">Abmelden</button>
              </div>
            </div>
          )}
          ...
          <div className="mt-4 pt-3.5 border-t border-ctp-surface0">
            ...
            <div className="font-mono text-base text-ctp-subtext1">
              Widget {appVersion || "unbekannt"} · Server {serverVersion ?? "unbekannt"}
            </div>
            {laeuftAuseinander && (
              <div className="text-sm text-ctp-yellow mt-1.5 leading-normal">
            ...
            <div className="flex items-center gap-3 flex-wrap mt-2.5">
              <span className={`text-sm leading-normal flex-1 min-w-[200px] ${
              ...
              {update.phase !== "unsupported" && (
                <button onClick={onCheckUpdate}
                  ...
                  className="rounded-lg px-3.5 py-[7px] text-sm font-semibold whitespace-nowrap transition-colors border border-ctp-surface1 text-ctp-subtext1 hover:text-ctp-text hover:bg-ctp-surface0 disabled:text-ctp-overlay0 disabled:hover:bg-transparent disabled:cursor-not-allowed">
                  Nach Updates suchen
                </button>
              )}
            </div>
