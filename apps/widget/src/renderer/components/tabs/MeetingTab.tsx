  ...
  return (
    <div className={`bg-ctp-mantle border border-ctp-surface0 rounded-[10px] px-[18px] py-4 mb-3 border-l-[3px] ${statusBorder(worstStatus(c))}`}>
      <div className="flex items-baseline gap-2.5 flex-wrap">
        <span className="font-mono text-md font-semibold text-ctp-text break-all">{c.filePath}</span>
        <span className="text-sm text-ctp-subtext0">{c.commitShort} · {c.authorName}</span>
        <span className="flex-1" />
        <span onClick={() => onOpen(c.id)} className="text-sm text-ctp-blue cursor-pointer hover:underline whitespace-nowrap">Änderung ansehen →</span>
      </div>
      <div className="text-base text-ctp-subtext1 mt-1">{c.summary}</div>
...
export function MeetingTab({ meeting, guardians, onOpen }:
  ...
  const pendingHint = counts.offen > 0 && (
    <div className="text-sm text-ctp-subtext1 bg-ctp-surface0/50 border border-ctp-surface1 rounded-lg px-3 py-2 inline-flex w-fit">
    ...
    <div className="flex-1 overflow-y-auto px-8 py-6">
      <div className="max-w-[820px] mx-auto">
        <div className="flex items-baseline gap-3.5 flex-wrap">
          <span className="text-2xl font-bold text-ctp-text">Offene Punkte</span>
          <span className="text-base text-ctp-subtext0">{parts.join(" · ")}</span>
        </div>
