  ...
  return (
    <div className={`border-l-[3px] ${a.rand} pl-3 min-w-0`}>
      <div className="flex items-baseline gap-2 mb-1.5">
        {!einzeln && <span className={`text-xs tracking-[0.08em] font-semibold ${a.text}`}>{a.titel}</span>}
        {masse && <span className="text-xs text-ctp-subtext0 font-mono">{masse}</span>}
      </div>
      ...
      {stand.status === "da" && (
        <>
          ...
          <span className="block text-xs text-ctp-overlay0 mt-1">Klicken zum Vergrößern</span>
        </>
      )}
    </div>
  );
