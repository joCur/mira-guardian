  ...
  return (
    <div className="w-screen h-screen bg-ctp-base flex flex-col overflow-hidden">
      ...
      <div style={{ WebkitAppRegion: "drag" } as any}
        ...
        <span className="text-base font-semibold text-ctp-text">Memory-Bank Hüter</span>
        <div className="flex gap-0.5 ml-3">
          {TABS.map(t => (
            <div key={t.id} onClick={() => onTab(t.id)} style={{ WebkitAppRegion: "no-drag" } as any}
              className={`px-3.5 py-1 rounded-[7px] text-base cursor-pointer transition-colors ${
        ...
        {!isMac && (
          <div onClick={onClose} style={{ WebkitAppRegion: "no-drag" } as any}
            className="w-[26px] h-[26px] rounded-[7px] flex items-center justify-center text-lg text-ctp-subtext0 cursor-pointer hover:bg-ctp-surface0 hover:text-ctp-text transition-colors">✕</div>
        )}
      </div>
      {children}
    </div>
